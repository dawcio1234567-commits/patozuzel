    ...
    <div class="p-3">
     ...
     <div class="space-y-2">
      ${RC.options.map(o=>{
        ...
        <b${o.id==='ajs'?' class="blink" style="color:#fdba74"':''}>${esc(o.l)}</b>${o.ch!=null?` <span class="${o.ch>=60?'text-emerald-400':o.ch>=35?'text-yellow-400':'text-red-400'}">— ${o.ch}%</span>`:''}${moveNote(o.id)}
     ...
     ${RC.sim&&RC.sim.show?`<div class="mt-3 pt-3" style="border-top:1px solid #3f3f46">
      ...
      <div class="space-y-2">
       <button onclick="liveAct('simfall')" class="btn-d w-full text-left px-3 py-2 text-[12px]">
         <b>SYMULUJ UPADEK</b> <span style="color:#fca5a5">— ${RC.sim.fallRed}% czerwonej kartki dla CIEBIE, reszta: powtórka biegu w pełnym składzie</span>
       ...
       <button onclick="liveAct('simdef')" class="btn-d w-full text-left px-3 py-2 text-[12px]">
         <b>SYMULUJ DEFEKT</b> <span style="color:#fca5a5">— 100% zera i kodu „d", 0% ryzyka kartki, pewna bura w parku maszyn</span>
